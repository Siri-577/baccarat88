...
test("Visual Cut Card is a single table-level presentation DOM, separate from Shoe equipment",()=>{assert.equal((html.match(/id="visual-cut-card"/g)||[]).length,1);assert.match(html,/id="cut-card-presentation" class="cut-card-presentation" hidden[\s\S]*?id="visual-cut-card"/);assert.doesNotMatch(html,/class="[^"]*shoe-visual[^"]*"[\s\S]*?id="visual-cut-card"/);});
...
test("Visual Cut Card uses a 400ms enter, 2000ms central hold, and 400ms exit",()=>{assert.equal(app.CUT_CARD_ENTER_MS,400);assert.equal(app.CUT_CARD_HOLD_MS,2000);assert.equal(app.CUT_CARD_EXIT_MS,400);assert.equal(app.CUT_CARD_EVENT_MS,2800);assert.match(css,/\.cut-card-presentation\.is-presenting\s*\{[^}]*2800ms/);assert.match(css,/@keyframes cut-card-presentation[\s\S]*?14\.2857%[\s\S]*?85\.7143%/);});
test("Visual Cut Card plays once per visible run and becomes truly hidden after the presentation",()=>{assert.match(source,/if \(!presentation \|\| !presentation\.hidden\) return/);assert.match(source,/presentation\.hidden = true/);assert.match(source,/presentation\.classList\.remove\("is-presenting"\)/);});
...
test("Mobile Cut Card is a readable central event while preserving V0.7.4 equipment safety variables",()=>{const mobile=css.slice(css.indexOf("@media (max-width: 600px)"));assert.match(mobile,/\.cut-card-presentation\s*\{\s*top:\s*32%/);assert.match(mobile,/\.visual-cut-card\s*\{[^}]*width:\s*60px[^}]*height:\s*90px/);assert.doesNotMatch(mobile,/width:\s*29px[^}]*height:\s*46px/);assert.match(mobile,/--mobile-equipment-height:\s*70px/);assert.match(mobile,/--mobile-player-clearance:\s*82px/);});
test("Central Cut Card presentation enters from the right and exits to the left without interaction",()=>{assert.match(css,/@keyframes cut-card-presentation[\s\S]*?translateX\(40vw\)[\s\S]*?translateX\(-40vw\)/);assert.match(css,/\.cut-card-presentation\s*\{[^}]*pointer-events:\s*none/);assert.match(css,/\.casino-table\s*\{[^}]*overflow:\s*hidden/);});
